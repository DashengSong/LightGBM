...
class Bin {
  ...
  */
  ...
  virtual void InitSplit(int num_features, int cur_feature, BinType bin_type,
                         uint32_t min_bin, uint32_t max_bin,
                         uint32_t default_bin, uint32_t most_freq_bin,
                         MissingType missing_type) const = 0;
  ...
  inline data_size_t Split(int sub_feature, const uint32_t* threshold,
    ...
    return split_funcs_[sub_feature](threshold, num_threshold, default_left,
 ...
 protected:
  mutable std::vector<std::function<data_size_t(
